...
public class CookService {
        ...
        try {
            ...
            if (null != result) {
                ...
                if (data.getString("resultcode").equals("200")) {
                    ...
                    for (int i = 0; i < jsonArray.length(); i++) {
                        JSONObject en = jsonArray.getJSONObject(i);
                        System.out.println("pn:" + pn + "----id:" + en.getInt("id"));
    ...
    public void saveCC(int pn, CookStyle style) {
        ...
        try {
            String result = "";
            String url = "http://apis.juhe.cn/cook/index?" + "key=d10fb4ba2667d9c3118a101cf8edba20&cid=" + style.getTmpId() + "&rn=30&pn=" + pn;
            HttpClient httpClient = HttpClientBuilder.create().build();
            HttpGet get = new HttpGet(url);
            HttpResponse response = httpClient.execute(get);
            if (response.getStatusLine().getStatusCode() == 200) {
