 ...
 */
public class DescribeBucket implements Sender {
    public DescribeBucket() {
    ...
    public UFileResponse send(UFileClient ufileClient, UFileRequest request) {
        ...
        try {
            uri = "http://" + request.getBucketName() + UFileConfig.getInstance().getDownloadProxySuffix()
                    ...
                    + "?" + URIUtil.encodeWithinQuery("Action", UTF8)
                    + "=" + URIUtil.encodeWithinQuery("DescribeBucket", UTF8)
                    + "?" + URIUtil.encodeWithinQuery("BucketName", UTF8)
                    + "=" + URIUtil.encodeWithinQuery("shonve", UTF8);
        } catch (Exception e) {
            e.printStackTrace();
        }
